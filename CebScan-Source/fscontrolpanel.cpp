...
FSControlPanel::~FSControlPanel()
{
   ...
   FSController::getInstance()->vertical->enable();
    delete ui;
}
...
void FSControlPanel::on_stepUpButton_clicked()
{
    FSController::getInstance()->vertical->setDirection(FS_DIRECTION_CCW);
    QString str = ui->StepsVertical->text();
    double degrees = str.toDouble();
    FSController::getInstance()->vertical->turnNumberOfSteps(degrees);
}

void FSControlPanel::on_stepDownButton_clicked()
{
    FSController::getInstance()->vertical->setDirection(FS_DIRECTION_CW);
    QString str = ui->StepsVertical->text();
    double degrees = str.toDouble();
    FSController::getInstance()->vertical->turnNumberOfSteps(degrees);
}
...
void FSControlPanel::on_GetLaserLines_clicked()
{
 ...
 //   FSController::getInstance()->laser->enable();
    ...
    qDebug() << "pressed";
    cv::Mat shot = CSVision::subLaser2(laserOffFrame, laserOnFrame);
    cv::resize(shot,shot,cv::Size(800,600));
    cv::imshow("Laser Frame",shot);
    cv::waitKey(0);
    cvDestroyWindow("Laser Frame");
